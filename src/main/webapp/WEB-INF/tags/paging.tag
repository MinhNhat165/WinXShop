	...
	<ul class="pagination justify-content-start">
		<c:if test="${!pagedListHolder.firstPage}">
			<li class="page-item"><a class="page-link"
				...
				href="<%=StringUtils.replace(pagedLink, "~", String.valueOf(pagedListHolder.getPage() - 1))%>"><i
					class="fa-regular fa-chevron-left"></i></a></li>
		</c:if>
			...
			<c:choose>
				<c:when test="${pagedListHolder.page == i}">
					<li class="page-item active"><a class="page-link btn"
						style="background: #046963" href="#">${i+1}</a></li>
				</c:when>
				<c:otherwise>
